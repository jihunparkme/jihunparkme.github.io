...
- **BootstrapClassLoader**: JAVA_HOME\lib 에 있는 코어 자바 API 제공
  - 최상위 우선순위를 가진 클래스 로더
- **PlatformClassLoader**: JAVA_HOME\lib\ext 폴더 또는 java.ext.dirs 시스템 변수에 해당하는 클래스를 읽음
- **AppClassLoader**: 애플리케이션 클래스 패스(애플리케이션 실행 시 주는 -classpath 옵션 또는 java.class.path 환경 변수의 값에 해당하는 위치)에서 클래스를 읽음

최상위 클래스 로더부터 클래스를 참색하는데 모든 클래스 로더가 클래스를 찾지 못 한다면 ClassNotFoundException 발생

`Linking`
- Verify, Prepare, Reolve(optional) 세 단계로 나눠져 있음
