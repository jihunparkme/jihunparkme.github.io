...
자바에서 비동기(Asynchronous) 프로그래밍이 가능한 인터페이스
- Future 로도 어느정도 가능하지만, 하기 어려운 작업들이 다수 존재
  - Future 를 외부에서 완료 불가
    - 취소, get() 타임아웃 설정은 가능
  - 블로킹 코드(get())를 사용하지 않고서는 작업이 끝났을 때 콜백 실행 불가
  - 여러 Future 조합 불가
    - ex. Event 정보 조회 후 Event 참석 회원 목록 조회하기
  - 예외 처리용 API 제공 X
...
- 원하는 Executor(쓰레드풀)를 사용해서 실행 가능
