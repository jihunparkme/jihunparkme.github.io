...
- **IoC(Inversion of Control, 제어의 역전)**
  - `프로그램의 제어 흐름을` 직접 제어하는 것이 아니라 `외부에서 관리`하는 것 (ex. AppConfig.java)
...
- 스프링 빈의 이벤트 라이프사이클
  ...
  - 스프링 컨테이너 -> 생성 스프링 빈 생성 -> 의존관계 주입 -> 초기화 콜백( 빈이 생성되고, 빈의 의존관계 주입이 완료된 후 호출) -> 사용 -> 소멸전 콜백(빈이 소멸되기 직전에 호출) -> 스프링 -> 종료

- 객체의 생성과 초기화를 분리하자.

  - 생성자는 객체 생성에 책임을, 초기화는 생성된 값들을 활용해서 커넥션 열결과 같은 무거운 동작을 수행
