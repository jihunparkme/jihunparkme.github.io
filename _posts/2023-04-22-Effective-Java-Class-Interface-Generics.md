...
>
> Set\<Object\>는 어떤 타입의 객체도 저장할 수 있는 매개변수화 타입이고, Set\<?\>는 모종의 타입 객체만 저장할 수 있는 와일드카드 타입니다.
>
...
- 다른 인스턴스로 넣어도 아무 오류 없이 컴파일되고 실행되지만, 꺼내지 전에는 오류를 알아내지 못 한다.
- 이 경우 코드 전체를 훑어봐야 하는 고생이 필요하다.
